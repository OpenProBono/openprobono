...
# from app import db
# from app.models import BotRequest, ChatModelParams, EngineEnum, OpenAIModelEnum, User, EvalDataset, EvalSession, LabelingAspect, LabelingType, LabeledEvalDataset, LabeledEvalSession
...
#         retrieved_dataset = db.get_run(dataset1_id)
...
#         retrieved_dataset_with_sessions = db.get_run(dataset_with_sessions_id)
...
# def test_labeled_eval_dataset_operations() -> None:
#     """Test storing, retrieving, and updating labeled evaluation datasets with multiple aspects."""
#     # Create test user
#     user = User(firebase_uid="test_user_1", email="test1@example.com")
...
#     labeled_dataset_id = f"test_labeled_dataset_{uuid.uuid4()}"
    
#     try:
#         # Create a test dataset with sessions
#         sessions = [
#             EvalSession(
#                 input_idx=0,
#                 bot_idx=0,
...
#         dataset = EvalDataset(
#             name="Test Dataset",
#             description="A test dataset for labeling",
#             inputs=["Question 1"],
#             bot_ids=[bot1_id, bot2_id],
#             sessions=sessions,
#             user=user
...
#         # Store the dataset
#         db.store_eval_dataset(dataset, dataset_id)
        
#         # Create labeling aspects
#         labeling_aspects = [
#             LabelingAspect(
#                 aspect_id="accuracy",
#                 name="Accuracy",
#                 description="How accurate is the response?",
#                 type=LabelingType.score
#             ),
#             LabelingAspect(
#                 aspect_id="helpfulness",
#                 name="Helpfulness",
#                 description="How helpful is the response?",
#                 type=LabelingType.thumbs
#             ),
#             LabelingAspect(
#                 aspect_id="preference",
#                 name="Preference Ranking",
#                 description="Rank the responses in order of preference",
#                 type=LabelingType.rank
#             )
#         ]
...
#             name="Labeled Test Dataset",
#             description="A test labeled dataset",
...
#         db.store_labeled_eval_dataset(labeled_dataset, labeled_dataset_id)
        
#         # Test retrieving the labeled dataset
#         retrieved_dataset = db.get_labeled_dataset(labeled_dataset_id)
#         assert retrieved_dataset is not None
#         assert retrieved_dataset.name == labeled_dataset.name
#         assert retrieved_dataset.description == labeled_dataset.description
#         assert len(retrieved_dataset.labeling_aspects) == 3
#         assert len(retrieved_dataset.sessions) == 2
        
#         # Test updating a labeled session
#         session_id = labeled_sessions[0].session_id
#         aspect_ratings = {
#             "accuracy": 4.5,  # Score for accuracy
#             "helpfulness": True,  # Thumbs up for helpfulness
#             "preference": 1  # Rank 1 for preference
#         }
#         notes = "This is a good response"
        
#         # Update the session
#         success = db.update_labeled_session(
#             dataset_id=labeled_dataset_id,
...
#         updated_dataset = db.get_labeled_dataset(labeled_dataset_id)
#         assert updated_dataset is not None
        
#         # Find the updated session
#         updated_session = None
#         for session in updated_dataset.sessions:
#             if session.session_id == session_id:
#                 updated_session = session
#                 break
        
#         assert updated_session is not None
#         assert updated_session.labeled is True
#         assert updated_session.notes == notes
        
#         # Check that the aspect ratings were updated correctly
#         for aspect in updated_session.aspects:
#             if aspect.aspect_id == "accuracy":
#                 assert aspect.score_value == 4.5
#             elif aspect.aspect_id == "helpfulness":
#                 assert aspect.thumbs_value is True
#             elif aspect.aspect_id == "preference":
#                 assert aspect.rank_value == 1
        
#         # Check that progress was updated
#         assert updated_dataset.progress == 50.0  # 1 out of 2 sessions labeled
#         assert updated_dataset.completed is False
        
#         # Update the second session
#         session_id = labeled_sessions[1].session_id
#         aspect_ratings = {
#             "accuracy": 3.5,
#             "helpfulness": False,
#             "preference": 2
#         }
#         notes = "This response could be improved"
...
#         # Check that progress was updated
#         assert updated_dataset.progress == 100.0  # 2 out of 2 sessions labeled
#         assert updated_dataset.completed is True
        
#         # Test retrieving user's labeled datasets
#         user_datasets = db.get_user_labeled_datasets(user)
#         assert labeled_dataset_id in user_datasets
        
#     finally:
#         # Clean up test data
#         db.db.collection(db.RUN_COLLECTION + db.DB_VERSION).document(dataset_id).delete()
#         db.db.collection(db.LABELED_EVAL_DATASET_COLLECTION + db.DB_VERSION).document(labeled_dataset_id).delete()
