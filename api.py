...
async def lifespan(app: FastAPI):
    ...
    # Flush all events to be sent to Langfuse on shutdown and terminate all Threads gracefully. This operation is
    # blocking.
    langfuse.flush()
            ...
            Body(
                openapi_examples={
                    "call a bot using history": {
                        ...
                        "description": "Returns: {message: 'Success', output: ai_reply, bot_id: the bot_id which was "
                                       "used}",
                        "value": {
                            "history": [["hi!", ""]],
            ...
            Body(
                openapi_examples={
                    "fetch chat history via session": {
                        ...
                        "description": "Returns: {message: 'Success', history: list of messages, bot_id: the bot_id "
                ...
                openapi_examples={
                            ...
                            "search_tools": [
                                {
                                    ...
                                    "prompt": "Useful for when you need to answer questions or find resources about "
                                              "government and laws.",
                                },
                                {
                                    "name": "case-search",
                    ...
                    "full descriptions of every parameter": {
                        ...
                        "value": {
                            ...
                            "model": "model to be used, currently only openai models, default is gpt-3.5-turbo-0125",
                            "search_tools": [
                                {
                                    "name": "name for tool",
                                    "method": "which search method to use, must be one of: serpapi, dynamic_serpapi, "
                                              "google, courtlistener",
                                    "prefix": "where to put google search syntax to filter or whitelist results, "
                                              "but is also just generally a prefix to add to query passed to tool by "
                                              "llm",
                            ...
                            "engine": "which library to use for model calls, must be one of: langchain, openai. "
...
def upload_files(files: list[UploadFile], session_id: str, summaries: list[str] = None):
    if not summaries:
        ...
        return {
            "message": f"Failure: did not find equal numbers of files and summaries, instead found {len(files)} files "
                       f"and {len(summaries)} summaries."
        }
    failures = []
    for i, file in enumerate(files):
        ...
        if result["message"].startswith("Failure"):
            failures.append(
                f"Upload #{i + 1} of {len(files)} failed. Internal message: {result['message']}"
            )
...
def delete_file(filename: str, session_id: str):
    return delete_expr(
        SESSION_PDF, f"source=='{filename}' and session_id=='{session_id}'"
    )
...
def vectordb_upload_site(
        site: str, collection_name: str, description: str, api_key: str
):
...
#         summaries = [file.filename for file in files]
#     elif len(files) != len(summaries):
#         return {"message": f"Failure: did not find equal numbers of files and summaries, instead found {len(files)}"
#                                               "files and {len(summaries)} summaries."}
#     failures = []
#     for i, file in enumerate(files):
